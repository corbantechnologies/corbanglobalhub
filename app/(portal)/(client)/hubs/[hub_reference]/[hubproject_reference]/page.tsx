...
import { ArrowLeft, Loader2, FolderKanban, Activity, Link as LinkIcon, DollarSign, Search, CheckCircle2, XCircle } from "lucide-react";
import Link from "next/link";
import { cn } from "@/lib/utils";
import { use, useState } from "react";

export default function ClientHubProjectDetailPage({ params }: { params: Promise<{ hub_reference: string, hubproject_reference: string }> }) {
  ...
  if (!project) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] text-slate-500">
        ...
        <Link href={`/hubs/${hub_reference}`} className="text-blue-600 hover:underline mt-2">
          Return to Hub
        </Link>
      </div>
    );
