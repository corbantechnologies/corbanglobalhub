...
export default function ClientHubDetailPage({ params }: { params: Promise<{ hub_reference: string }> }) {
  ...
  if (!hub) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] text-slate-500">
        ...
        <Link href="/hubs" className="text-blue-600 hover:underline mt-2">Return to Hubs</Link>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 md:p-10 space-y-8 mx-auto">
        ...
        <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        <Link href={`/hubs/${hub.reference}/${project.reference}`} className="text-lg font-semibold text-slate-900 hover:text-blue-600 transition-colors">
                          {project.name}
                        </Link>
                        {!project.is_active && (
          ...
          <div className="pt-6 border-t border-slate-200">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href="/billing" className="text-sm text-blue-600 hover:text-blue-700 font-medium">View All</Link>
            </div>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {hubInvoices.map((invoice) => (
                      <tr key={invoice.reference} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-6 py-4">
                          <Link href={`/billing/${invoice.reference}`} className="font-semibold text-slate-900 hover:text-blue-600 transition-colors">
                            {invoice.code}
                          </Link>
      ...
      <SlideOver
        ...
        title="Update Hub"
        description="Modify your hub details and billing information."
      >
        <UpdateHubForm 
          hub={hub} 
