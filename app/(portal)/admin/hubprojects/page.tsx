...
import UpdateHubProjectForm from "@/forms/hubprojects/UpdateHubProject";
import { HubProject } from "@/services/hubprojects";
import { Plus, Search, Edit2, Loader2, FolderKanban } from "lucide-react";
import { cn } from "@/lib/utils";

export default function HubProjectsPage() {
  const { data: projects, isLoading } = useFetchHubProjects();
  const [searchTerm, setSearchTerm] = useState("");
  
  // SlideOver state
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const [editingProject, setEditingProject] = useState<HubProject | null>(null);

  const filteredProjects = projects?.filter(project => 
    project.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    project.code.toLowerCase().includes(searchTerm.toLowerCase()) ||
    project.hub.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="p-6 md:p-10 space-y-6 mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Hub Projects</h1>
          <p className="text-slate-500 mt-1">Manage infrastructure and migration projects</p>
        </div>
        <button
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/80 border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500 font-semibold">
                  ...
                  <th className="px-6 py-4">Target Hub</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredProjects?.map((project) => (
                  <tr key={project.reference} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-6 py-4">
                      <p className="font-semibold text-slate-900">{project.name}</p>
                      <p className="text-sm text-slate-500 line-clamp-1 max-w-xs">{project.description}</p>
                    </td>
                    <td className="px-6 py-4">
      ...
      <SlideOver
        ...
        description="Modify project details and status."
      >
        {editingProject && (
          <UpdateHubProjectForm 
